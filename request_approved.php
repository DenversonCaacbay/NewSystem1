<?php 
    ...
    $requests_approved = $residentbmis->view_request_approved($userdetails['id_resident']);
...
<div class="container-fluid request--container">
    ...
    if(is_array($requests_approved)) {
        ...
        $all_requests = [];
        foreach($requests_approved as $request_data) {
                ...
                <form method="POST">
                    <div class="card mt-3 mb-3">
                        <div class="card-header bg-primary text-light d-flex justify-content-between align-items-center">
                            Date Approved: <?= ucfirst(date("F d, Y", strtotime($request['created_at']))); ?>
                            <?php 
                            // Display common request fields
                            if($request['id_brgyid']) { ?>
                                <input type="hidden" name="id_brgyid" value="<?= $request['id_brgyid'] ?>"/>
                                <button class="btn btn-warning py-0" name="file_request" >Affidavit Loss</button>
                            <?php } ?>
                        </div>
                        <div class="card-body">
                            <?php 
