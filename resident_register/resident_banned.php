	...
	<table class="table table-hover text-center table-bordered table-responsive" >
		...
		<tbody>       
			<?php
				...
				or  `bdate` LIKE '%$keyword%' or  `bplace` LIKE '%$keyword%' or  `nationality` LIKE '%$keyword%' or  `family_role` LIKE '%$keyword%' or  `role` LIKE '%$keyword%' or  `email` LIKE '%$keyword%' WHERE verified='No'");
				$stmnt->execute();
                    ...
                    <tr>
                        ...
                        <td width="20%">    
                            <form action="" method="post">
                                <!-- <a href="update_resident_form.php?id_resident=<?= $view['id_resident'];?>" class="btn btn-success">  Update </a> -->
                                <input type="hidden" name="id_resident" value="<?= $view['id_resident'];?>">
                                <a class="btn btn-primary" href="admn_view_banned_details.php?id_resident=<?= $view['id_resident'];?>" name=""> View </a>
                            </form>
                        </td>
                    </tr>
